...
export default function AbstractQuestion() {
    ...
    useEffect(() => {
        ...
        const restoredValues = stored
            .split("\n")
            .map((item) => item.trim())
            .filter((item): item is (typeof abstractOptions)[number] =>
                abstractOptions.includes(item as (typeof abstractOptions)[number])
            );
    ...
    useEffect(() => {
        ...
        writeHearingAnswer("abstract", selectedWords.join("\n"));
    ...
    return (
        <Field>
            <FieldContent>
                <FieldTitle className="main-content-label mb-1 text-l font-bold">抽象的</FieldTitle>
                <FieldDescription className="mb-3">
                    イメージに近い単語を 4 つまで選択してください。
                </FieldDescription>
                <div className="grid gap-4 md:grid-cols-3">
                    {abstractOptions.map((word) => (
                        <button
                            key={word}
                            type="button"
