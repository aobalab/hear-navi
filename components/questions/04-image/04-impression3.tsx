...
export default function Impression3Question() {
    ...
    useEffect(() => {
        const stored = readHearingAnswers().impression3 ?? "";
        const restoredValues = stored
            .split("\n")
            .map((item) => item.trim())
            .filter((item): item is (typeof impression3Options)[number] =>
                impression3Options.includes(item as (typeof impression3Options)[number])
            );
    ...
    useEffect(() => {
        ...
        writeHearingAnswer("impression3", selectedWords.join("\n"));
